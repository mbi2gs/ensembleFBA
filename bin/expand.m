function [model, rxnDatabase, fromUthresh, fromXthresh, feasible] = phen2net_expand_hpc(universalRxnSet,growthConditions,nonGrowthConditions,biomassFn,Urxns2set,Uset2,Xrxns2set,Xset2,verbose,stochast,rndSeed)
%-------------------------------------------------------------------------- 
% Implementation of the Phen2Net algorithm 
%
% Version iter1 uses an iterative process to get at the global
% optimization, using first the continuous variable optimization to select
% a set of reactions that allow growth in all growth conditions. Next, a 
% binary optimization removes reactions so that the model cannot grow under 
% any negative growth conditions. Then it returns to the continous 
% optimization to add reactions to allow growth in all growth conditions 
% (excluding reactions removed in the binary optimization). It continues 
% until all growth and non-growth conditions are satisfied.
% 
% This is the expansion step which uses continuous variables to
% constrain the absolute biomass flux value through individual conditions.
%
