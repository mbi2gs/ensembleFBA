function [model, rxnDatabase, fromDecthresh, fromGCthresh] = phen2net_SEED_trim_active(universalRxnSet,growthConditions,nonGrowthConditions,biomassFn,decisionRxns,verbose)
%-------------------------------------------------------------------------- 
% Implementation of the Phen2Net algorithm 
%
% This is the trim step which uses binary optimization to
% remove a minimal number of reactions in order to prevent growth in 
% the non-growth conditions (while allowing growth through as many 
% growth conditions as possible). This version only includes binary
% variables for the subset of reactions indicated by the user.
%
...
try
    ...
    newModel.rxns = [universalRxnSet.rxns(rulog); Ex_names; {'Biomass'}];
    newModel.rxnNames = [universalRxnSet.rxnNames(rulog); Ex_names; {'Biomass'}];
    newModel.rev = double([universalRxnSet.rev(rulog); zeros(length(Ex_names),1); 0]);
    newModel.S = double([universalRxnSet.S(:,rulog) X biomassFn(:)]);
    ...
    newRDB.rxns = universalRxnSet.rxns(rulog);
    newRDB.rxnNames = universalRxnSet.rxnNames(rulog);
    newRDB.rev = universalRxnSet.rev(rulog);
    newRDB.S = universalRxnSet.S(:,rulog);
    newRDB.X = X;
