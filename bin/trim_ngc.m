function [model, rxnDatabase, fromNGCthresh] = trim_ngc(rxnSet,growthConditions,nonGrowthConditions,biomassFn,verbose)
%-------------------------------------------------------------------------- 
% trim_ngc - Removes inconsistent non-growth conditions.
%
% Inputs:
...
%     fromNGCthresh - logical vector indicating the non-growth conditions kept (1) and removed (0)
%
...
try
    ...
    newModel.rxns = [rxnSet.rxns; Ex_names; {'Biomass'}];
    newModel.rxnNames = [rxnSet.rxnNames; Ex_names; {'Biomass'}];
    newModel.rev = double([rxnSet.rev; zeros(length(Ex_names),1); 0]);
    newModel.S = double([rxnSet.S X biomassFn(:)]);
    ...
    newRDB.rxns = rxnSet.rxns;
    newRDB.rxnNames = rxnSet.rxnNames;
    newRDB.rev = rxnSet.rev;
    newRDB.S = rxnSet.S;
    newRDB.X = X;
